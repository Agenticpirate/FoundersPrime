    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="rounded-sm border-3 border-black bg-white p-4 shadow-[3px_3px_0px_#111111]">
            <div className="text-xs font-bold uppercase text-gray-500 mb-1 font-mono">Time</div>
            <div className="text-lg font-bold text-black font-mono">{deal.stats.appTime}</div>
          </div>
          <div className="rounded-sm border-3 border-black bg-white p-4 shadow-[3px_3px_0px_#111111]">
            <div className="text-xs font-bold uppercase text-gray-500 mb-1 font-mono">Approval</div>
            <div className="text-lg font-bold text-black font-mono">{deal.stats.approval}</div>
          </div>
          <div className="rounded-sm border-3 border-black bg-white p-4 shadow-[3px_3px_0px_#111111]">
            <div className="text-xs font-bold uppercase text-gray-500 mb-1 font-mono">Difficulty</div>
            <div className="text-lg font-bold text-green-600 font-mono">{deal.stats.difficulty}</div>
          </div>
          <div className="rounded-sm border-3 border-black bg-white p-4 shadow-[3px_3px_0px_#111111]">
            <div className="text-xs font-bold uppercase text-gray-500 mb-1 font-mono">Success</div>
            <div className="text-lg font-bold text-blue-600 font-mono">{deal.stats.successRate}</div>
          </div>
        </div>
      ...
      <div className="lg:col-span-1">
        <div className="sticky top-24 space-y-6">
          ...
          {/* Apply Box */}
          <div className="rounded-sm border-4 border-black bg-white shadow-[6px_6px_0px_#111111] p-6">
            <div className="mb-4">
              <div className="text-sm font-bold uppercase text-gray-500 mb-2 font-mono">Deal Value</div>
              <div className="text-3xl font-black text-black font-mono mb-4">{displayValue}</div>
            </div>
            ...
            {isLoadingPro ? (
              <div className="w-full h-14 bg-gray-100 animate-pulse rounded-sm mb-3"></div>
            ) : (
              <a
                href={applicationUrl}
                target="_blank"
                rel="noopener noreferrer"
                onClick={handleApplyClick}
                className="w-full rounded-sm border-4 border-black bg-primary py-4 font-mono text-base font-bold uppercase tracking-wide text-black shadow-[4px_4px_0px_#111111] hover:bg-yellow-300 hover:shadow-[6px_6px_0px_#111111] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all mb-3 flex items-center justify-center gap-2 cursor-pointer"
              >
                {isPro ? 'Apply Now' : 'Apply Now'}
                <span className="material-symbols-outlined !text-[20px]">{isPro ? 'arrow_forward' : 'lock'}</span>
              </a>
            )}
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={handleSave}
                disabled={isSaving}
                className={`rounded-sm border-3 border-black py-2.5 font-mono text-sm font-bold uppercase hover:shadow-[3px_3px_0px_#111111] hover:translate-x-[-1px] hover:translate-y-[-1px] transition-all flex items-center justify-center gap-1 ${isSaved ? 'bg-yellow-400 text-black' : 'bg-white text-black hover:bg-gray-50'
              ...
              <div className="relative">
                <button
                  ...
                  className="w-full rounded-sm border-3 border-black bg-white py-2.5 font-mono text-sm font-bold uppercase text-black hover:bg-gray-50 hover:shadow-[3px_3px_0px_#111111] hover:translate-x-[-1px] hover:translate-y-[-1px] transition-all flex items-center justify-center gap-1"
                >
                  <span className="material-symbols-outlined !text-[18px]">ios_share</span> Share
                </button>
                {showShareMenu && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white border-3 border-black shadow-[4px_4px_0px_#111111] z-50">
                    <button
                      onClick={() => handleShare('copy')}
                      className="w-full px-4 py-2 text-left text-sm font-bold hover:bg-gray-100 flex items-center gap-2"
                    >
          ...
          {/* Verified Badge */}
          <div className="rounded-sm border-4 border-black bg-green-50 p-5 shadow-[4px_4px_0px_#111111]">
            <div className="flex items-start gap-3">
              <div className="h-10 w-10 rounded-full bg-green-400 border-3 border-black flex items-center justify-center shrink-0 shadow-[2px_2px_0px_#111111]">
                <span className="material-symbols-outlined text-black !text-[20px]">verified</span>
              </div>
              <div>
                <h4 className="font-bold font-mono text-base uppercase mb-2">Verified Deal</h4>
                <p className="text-sm text-green-900 mb-1 font-mono">
          ...
          {/* Quick Links */}
          <div className="rounded-sm border-4 border-black bg-white shadow-[4px_4px_0px_#111111] p-5">
            ...
            <ul className="space-y-3">
              <li>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 text-sm font-bold hover:text-primary hover:underline"
                >
              ...
              <li>
                <a
                  href={`https://www.${deal.provider.toLowerCase().replace(/\s+/g, '')}.com`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 text-sm font-bold hover:text-primary hover:underline"
                >
          ...
          {/* Help CTA */}
          <div className="rounded-sm bg-black text-white p-6 shadow-[6px_6px_0px_#111111] text-center border-4 border-black">
            <div className="inline-block p-3 rounded-full bg-yellow-400 border-3 border-white mb-4 text-black shadow-[3px_3px_0px_rgba(255,255,255,0.3)]">
            ...
            <button className="w-full py-3 bg-yellow-400 text-black font-bold font-mono text-sm uppercase rounded-sm hover:bg-yellow-300 shadow-[4px_4px_0px_rgba(255,255,255,0.2)] hover:shadow-[3px_3px_0px_rgba(255,255,255,0.2)] hover:translate-x-[1px] hover:translate-y-[1px] transition-all border-2 border-white">
              Get Expert Help
            </button>
          </div>
        </div>
