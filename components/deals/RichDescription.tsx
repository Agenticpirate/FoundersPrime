...
export default function RichDescription({ text }: { text: string }) {
  ...
  return (
    <div className="space-y-3 text-[13.5px] text-gray-700 leading-relaxed">
      {visible.map((block, i) => {
        if (block.type === 'heading') {
          return (
            <h3
              key={i}
              className="font-mono text-[11.5px] font-black uppercase tracking-[0.1em] text-black inline-flex items-center gap-2 pt-1 bg-accent-yellow/40 border-2 border-black px-2.5 py-1 rounded-sm"
            >
            ...
            <ul key={i} className="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-1.5">
              {block.items.map((item, j) => (
                <li key={j} className="flex items-start gap-2 text-[12.5px] leading-snug">
                  <span className="material-symbols-outlined text-emerald-600 flex-shrink-0 !text-[15px] mt-0.5">
                    check_circle
                  </span>
