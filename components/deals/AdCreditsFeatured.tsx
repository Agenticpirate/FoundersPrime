...
export default function AdCreditsFeatured() {
  const featuredAdCredits = [
    {
      id: 'google-ads-startup',
      title: 'Google Ads',
      gradient: 'from-blue-500 to-green-500',
      icon: 'search',
      tags: [
        { label: 'SEARCH', color: 'bg-blue-100 text-blue-800 border-blue-900' },
    ...
    },
    {
      ...
      icon: 'groups',
      tags: [
        { label: 'SOCIAL', color: 'bg-purple-100 text-purple-800 border-purple-900' }
      ],
      value: '$15,000 credits',
      validity: 'New advertisers only',
      href: '/deals/facebook-ads-startup'
    },
    {
      id: 'linkedin-ads-startup',
      title: 'LinkedIn Ads',
      gradient: 'from-blue-700 to-blue-900',
      icon: 'work',
      tags: [
        { label: 'B2B', color: 'bg-gray-100' }
      ],
  ...
  return (
    <div className="mb-10 md:mb-14">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {featuredAdCredits.map((credit) => (
          <div key={credit.id} className="bg-white border-3 border-black shadow-[6px_6px_0px_0px_#1a1a1a] rounded-sm flex flex-col group hover:translate-y-[-4px] transition-transform">
            <div className={`h-32 bg-gradient-to-br ${credit.gradient} border-b-3 border-black relative overflow-hidden flex items-center justify-center p-6`}>
            ...
            <div className="p-5 flex-1 flex flex-col">
              ...
              <div className="space-y-3 mb-6 flex-1">
                <div className="flex items-start gap-2">
                  ...
                  <span className="font-mono text-sm font-bold">{credit.value}</span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="material-symbols-outlined text-black text-lg">info</span>
                  <span className="font-mono text-sm text-gray-600">{credit.validity}</span>
                </div>
              </div>
              
              <Link className="w-full" href={credit.href}>
                <button className="w-full py-2 text-sm flex items-center justify-center gap-2 bg-primary text-black border-3 border-black shadow-[4px_4px_0px_0px_#1a1a1a] font-mono font-bold rounded-sm hover:shadow-[2px_2px_0px_0px_#1a1a1a] hover:translate-x-[2px] hover:translate-y-[2px] transition-all group-hover:bg-black group-hover:text-white group-hover:border-black">
                  Get Credits <span className="material-symbols-outlined text-sm">arrow_forward</span>
                </button>
              </Link>
            </div>
          </div>
