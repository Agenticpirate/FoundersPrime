...
/**
 * Repeating rosette band used as a section rule, so brand colour carries down
 * the body instead of stopping at the header. Generated by
 * scripts/generate-email-art.mjs. Decorative, so alt is empty: a client with
 * images blocked shows nothing rather than stray words.
 */
function dividerBand(): string {
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:26px 0 0;">
      <tr>
        <td style="background:${BRAND.headerBar};line-height:0;font-size:0;">
          <img src="${emailAssetBase()}/email/divider-band.png" width="600" height="28" alt="" style="display:block;width:100%;max-width:600px;height:auto;border:0;outline:none;">
        </td>
      </tr>
    </table>`
}

/** Radial brand motif, used once per email beside an offer headline. */
function medallionImg(size = 76): string {
  return `<img src="${emailAssetBase()}/email/mandala-medallion.png" width="${size}" height="${size}" alt="" style="display:block;width:${size}px;height:${size}px;border:0;outline:none;">`
}

/**
 * Section label with an accent tab. Cheap way to repeat the brand colour at
 * every scroll stop without adding another image request.
 */
function sectionKicker(label: string): string {
  return `<table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin:22px 0 0;">
      <tr>
        <td style="background:${BRAND.accent};width:10px;line-height:0;font-size:0;">&nbsp;</td>
        <td style="padding-left:8px;font-family:'IBM Plex Mono',Consolas,Menlo,monospace;font-size:11px;font-weight:900;letter-spacing:1.2px;text-transform:uppercase;color:${BRAND.ink};">${escapeHtml(label)}</td>
      </tr>
    </table>`
}
...
function statStrip(
  ...
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:20px 0 0;border-top:4px solid ${BRAND.accent};border-bottom:2px solid ${BRAND.ink};background:${BRAND.paper};">
      <tr>
        ${stats
          .map(
            (s) =>
        ...
        .map(
          (t) =>
            `<td width="50%" style="padding:0 4px 8px 0;vertical-align:top;">
              <a href="${escapeHtml(t.url)}" style="display:block;border:2px solid ${BRAND.ink};border-left:6px solid ${BRAND.accent};background:${BRAND.surface};padding:11px 12px;text-decoration:none;">
...
/**
 * Checklist rows with an accent marker. A table rather than a <ul> because
 * Outlook ignores list-style images and Gmail strips ::marker styling, so the
 * square has to be a real cell to render everywhere.
 */
function bulletList(items: string[]): string {
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 18px;">${items
    .map(
      (item) =>
        `<tr>
          <td width="16" style="padding:7px 8px 8px 0;vertical-align:top;line-height:0;font-size:0;">
            <table role="presentation" cellspacing="0" cellpadding="0" border="0"><tr><td style="width:8px;height:8px;background:${BRAND.accent};border:1px solid ${BRAND.ink};line-height:0;font-size:0;">&nbsp;</td></tr></table>
          </td>
          <td style="padding:0 0 8px;font-size:15px;line-height:1.5;color:#374151;">${escapeHtml(item)}</td>
        </tr>`
    )
    .join('')}</table>`
}
...
export async function sendWelcomeEmail(
  ...
  const upgradeBlock = `
      ${dividerBand()}
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:0;border:2px solid ${BRAND.ink};border-top:0;background:${BRAND.paper};">
        <tr>
            ...
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
              <tr>
                <td style="vertical-align:top;padding-right:12px;">
                  ...
                  <p style="margin:0 0 8px;font-size:19px;font-weight:800;color:${BRAND.ink};line-height:1.25;">${
                    upgrade
                      ? escapeHtml(upgrade.pitch)
                      : 'You hold the top tier — here is the full ladder for reference.'
                  }</p>
                  <p style="margin:0;font-size:13px;color:${BRAND.subtle};line-height:1.5;">Both subscription plans start with a 30-day trial: ${escapeHtml(PLAN_OFFER.nextfounder.trial)} for Next&#39;Founder, ${escapeHtml(PLAN_OFFER.founder.trial)} for Founder. Legend is a single payment with nothing to renew.</p>
                </td>
                <td width="76" style="vertical-align:top;width:76px;">${medallionImg()}</td>
              </tr>
            </table>
          </td>
        </tr>
...
export async function sendSignupWelcomeEmail(
  ...
  const pricingUrl = `${appUrl()}/pricing#plans`
  const preferencesUrl = preferencesUrlFor(payload.userId)
  const subject = `Your account is live — ${OFFERS_TOTAL.toLocaleString('en-US')} verified startup deals inside`

  const freeIncludes = [
    `Search all ${OFFERS_TOTAL.toLocaleString('en-US')} verified offers by category, stage and region`,
    'Save what looks relevant to your dashboard and come back to it',
    `Preview the ${PROGRAMS_TOTAL} funding programs — grants, accelerators and incubators`,
  ]
  ...
  const html = emailShell({
    preheader: `Cloud credits, SaaS discounts, grants and student perks in one place. Your first 30 days cost ${PLAN_OFFER.nextfounder.trial}.`,
    body: `
      <p style="margin:0 0 18px;">${greeting(payload.firstName)}</p>
      ...
      ${sectionKicker('Pick where to start')}
      ${catalogTiles()}
      ${cta(`Browse all ${OFFERS_TOTAL.toLocaleString('en-US')} offers`, dealsUrl)}
            ...
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
              <tr>
                <td style="vertical-align:top;padding-right:12px;">
                  ...
                  <p style="margin:0 0 8px;font-size:19px;font-weight:800;color:${BRAND.ink};line-height:1.25;">Try the whole catalog for 30 days. ${escapeHtml(PLAN_OFFER.nextfounder.trial)} or ${escapeHtml(PLAN_OFFER.founder.trial)}.</p>
                  <p style="margin:0;font-size:13px;color:${BRAND.subtle};line-height:1.5;">One approved cloud credit usually covers the whole year. Members who work through the catalog save ${escapeHtml(CATALOG_COPY.typicalSaving)} in year one.</p>
                </td>
                <td width="76" style="vertical-align:top;width:76px;">${medallionImg()}</td>
              </tr>
            </table>
      ...
      <p style="margin:24px 0 0;font-size:13px;color:${BRAND.subtle};">Expiring soonest: <a href="${escapeHtml(`${appUrl()}/flash-deals`)}" style="color:${BRAND.ink};font-weight:700;">${CATALOG.flashDeals} flash deals</a>. Still studying? <a href="${escapeHtml(`${appUrl()}/student-benefits`)}" style="color:${BRAND.ink};font-weight:700;">${CATALOG.studentPerks.toLocaleString('en-US')} student perks</a>. Not sure where to begin? <a href="${escapeHtml(`${appUrl()}/resources`)}" style="color:${BRAND.ink};font-weight:700;">founder resources</a>.</p>
      <p style="margin:16px 0 0;font-size:13px;color:#6b7280;">Your <a href="${escapeHtml(dashboardUrl)}" style="color:#374151;font-weight:700;">dashboard</a> keeps saved deals in one place. Questions? Reply to this email — it reaches a person, not a queue.</p>
    `,
    footer: transactionalFooter(
  ...
  const text = `${payload.firstName?.trim() ? `Hi ${payload.firstName.trim().split(/\s+/)[0]},` : 'Hi Founder,'}\n\nYour FoundersPrime account is live — ${OFFERS_TOTAL.toLocaleString(
    'en-US'
  )} verified offers, all re-checked before they go live.\n\nWhat is inside: ${CATALOG.founderDeals} founder deals (${
    CATALOG.cloudDeals
